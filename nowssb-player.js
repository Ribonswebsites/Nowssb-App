...
/* Authoritative spec renderer: one in-place player, real queue/audio state. */
(function () {
  var audio = null, repeat = 'off', shuffle = false, session = null;
  var levelNames = ['Whisper','Slow','Gentle','Steady','Clear','Natural','Confident','Fluent','Sharp','Mastery'];
  var rates = [.55,.65,.75,.85,.95,1,1.05,1.1,1.15,1.2];
  function esc(v) { return String(v == null ? '' : v).replace(/[&<>\"']/g, function (c) { return ({'&':'&amp;','<':'&lt;','>':'&gt;','\"':'&quot;',"'":'&#39;'})[c]; }); }
  function queue() { return (typeof PRACTICE_WORDS !== 'undefined' && Array.isArray(PRACTICE_WORDS)) ? PRACTICE_WORDS : []; }
  function index() { return (typeof _pwIdx !== 'undefined') ? _pwIdx : 0; }
  function word() { return queue()[index()] || {}; }
  function id(w) { return String(w.id || w.word || '').toLowerCase().replace(/[^a-z0-9_-]/g, '-'); }
  function getLevel(w) { var n = Number(localStorage.getItem('nowssb_level_' + id(w)) || 1); return n >= 1 && n <= 10 ? n : 1; }
  function sessions() { try { return JSON.parse(localStorage.getItem('nb_sessions_v2') || '[]'); } catch (_) { return []; } }
  function saveSessions(v) { try { localStorage.setItem('nb_sessions_v2', JSON.stringify(v)); } catch (_) {} }
  function statData() { var list = sessions().filter(function (x) { return x.completed; }), days = {}; list.forEach(function (x) { days[new Date(x.date).toDateString()] = true; }); var d = new Date(), streak = 0; while (days[d.toDateString()]) { streak++; d.setDate(d.getDate() - 1); } var sec = list.reduce(function (a, x) { return a + Number(x.durationSec || 0); }, 0); return { streak: streak, sessions: list.length, time: sec ? Math.floor(sec / 3600) + 'h ' + Math.floor(sec % 3600 / 60) + 'm' : '0m' }; }
  function icon(p) { return '<svg viewBox="0 0 24 24" aria-hidden="true"><path d="' + p + '"/></svg>'; }
  function render() {
    var body = document.getElementById('practiceBody'), w = word(); if (!body || !w) return; var s = statData(), name = esc(window._userName || (window._currentUser && window._currentUser.displayName) || 'NowssB'), title = esc((w.title || w.word || '').toUpperCase()), cat = esc((w.organ || (w.categories || [])[0] || 'MEDITATION').toUpperCase()), lvl = getLevel(w), list = queue(), next = list[index() + 1];
    body.innerHTML = '<div class="nb-player" id="nbPlayer"><div class="nb-stats"><div class="nb-stat"><span class="nb-icon">' + icon('M12 21c4 0 7-2.7 7-6.5 0-3.2-2.1-5.3-4.5-7.2.2 2-1 3.2-2.1 3.7.3-3.8-1.8-6.9-4.4-8.7.1 4.3-4 6.5-4 11.2C4 18.2 7.2 21 12 21Z') + '</span><span class="nb-stat-num" id="statStreak">' + s.streak + '</span><span class="nb-stat-label">STREAK DAYS</span></div><span class="nb-divider"></span><div class="nb-stat"><span class="nb-icon">' + icon('M5 19V12M10 19V7M15 19V10M20 19V4') + '</span><span class="nb-stat-num" id="statSessions">' + s.sessions + '</span><span class="nb-stat-label">SESSIONS</span></div><span class="nb-divider"></span><div class="nb-stat"><span class="nb-icon">' + icon('M12 3a9 9 0 1 0 0 18 9 9 0 0 0 0-18ZM12 7v5l3 2') + '</span><span class="nb-stat-num" id="statTime">' + s.time + '</span><span class="nb-stat-label">TIME PRACTICED</span></div></div><div class="nb-card"><div class="nb-header"><div class="nb-avatar">' + esc(name.slice(0, 2).toUpperCase()) + '</div><div class="nb-id"><div class="nb-name" id="userName">' + name + '</div><div class="nb-tagline">WORD · SOUND · MEANING</div><button class="nb-level" id="levelPill" type="button"><span id="levelNum">Level ' + lvl + '</span> <span>›</span></button></div><button class="nb-info" id="infoBtn" type="button">i</button></div><div class="nb-content"><div class="nb-content-top"><div class="nb-viz" id="viz"><span></span><span></span><span></span><span></span><span></span></div><div class="nb-content-actions"><button class="nb-icon-btn" id="favBtn" type="button" aria-label="Favorite">' + icon('M12 20s-7-4.35-9.5-8.5C.5 7.5 3 4 6.5 4c2 0 3.5 1.2 5.5 3.5C14 5.2 15.5 4 17.5 4 21 4 23.5 7.5 21.5 11.5 19 15.65 12 20 12 20z') + '</button><button class="nb-icon-btn" id="moreBtn" type="button">' + icon('M12 5h.01M12 12h.01M12 19h.01') + '</button></div></div><div class="nb-title" id="trackTitle">' + title + '</div><div class="nb-category" id="trackCategory">' + cat + '</div></div><div class="nb-progress-wrap"><input class="nb-progress" id="seek" type="range" min="0" max="1000" value="0"><div class="nb-times"><span id="timeElapsed">0:00</span><span id="timeTotal">0:00</span></div></div></div><div class="nb-tube"><button class="nb-tube-btn" id="btnShuffle" aria-label="Shuffle">' + icon('M3 6h3l7 12h6M3 18h3l3-5M16 6h5v5M14 10l7-4M16 18h5v-5') + '</button><button class="nb-tube-btn" id="btnPrev" aria-label="Previous">' + icon('M6 6h2v12H6zM20 6L10 12l10 6z') + '</button><button class="nb-tube-play" id="btnPlay" aria-label="Play">' + icon('M8 5v14l11-7z') + '</button><button class="nb-tube-btn" id="btnNext" aria-label="Next">' + icon('M16 6h2v12h-2zM4 6l10 6L4 18z') + '</button><button class="nb-tube-btn" id="btnRepeat" aria-label="Repeat">' + icon('M17 2l4 4-4 4M3 11V9a4 4 0 0 1 4-4h14M7 22l-4-4 4-4M21 13v2a4 4 0 0 1-4 4H3') + '</button></div><div class="nb-options"><button class="nb-opt active">' + icon('M4 10v4M9 6v12M14 3v18M19 8v8') + '<span>EQUALIZER</span></button><button class="nb-opt">' + icon('M12 3a9 9 0 1 0 0 18 9 9 0 0 0 0-18Z') + '<span>BASS BOOST</span></button><button class="nb-opt">' + icon('M12 3a9 9 0 1 0 0 18 9 9 0 0 0 0-18Z') + '<span>AMBIENCE</span></button><button class="nb-opt">' + icon('M12 3l8 4v6c0 5-3.5 7.5-8 8-4.5-.5-8-3-8-8V7z') + '<span>MORE</span></button></div><div class="nb-nextup">' + (next ? '<div class="nb-nextup-item"><div class="nb-nextup-art" style="background-image:url(\'' + esc(next.img || '') + '\')"></div><div class="nb-nextup-mid"><div class="nb-eyebrow">NEXT UP</div><div class="nb-nextup-title">' + esc((next.title || next.word || '').toUpperCase()) + '</div><div class="nb-nextup-sub">' + esc(next.organ || 'MEDITATION') + '</div></div><button class="nb-nextup-play" id="nextUpPlayBtn">' + icon('M8 5v14l11-7z') + '</button></div>' : '<div class="nb-nextup-empty">No more sessions queued</div>') + '</div><audio id="nbAudio" preload="metadata"></audio></div>';
    bind(w, lvl);
  }
  function bind(w, lvl) { var root = document.getElementById('nbPlayer'); audio = document.getElementById('nbAudio'); audio.src = '/api/audio?word=' + encodeURIComponent(id(w)) + '&level=' + lvl; session = { id: id(w), date: new Date().toISOString(), durationSec: 0, completed: false }; audio.addEventListener('play', function () { root.classList.add('is-playing'); }); audio.addEventListener('pause', function () { root.classList.remove('is-playing'); }); audio.addEventListener('loadedmetadata', function () { document.getElementById('timeTotal').textContent = clock(audio.duration); }); audio.addEventListener('timeupdate', function () { if (audio.duration) { document.getElementById('seek').value = audio.currentTime / audio.duration * 1000; document.getElementById('seek').style.setProperty('--val', audio.currentTime / audio.duration * 100 + '%'); } document.getElementById('timeElapsed').textContent = clock(audio.currentTime); if (session) session.durationSec = audio.currentTime; }); audio.addEventListener('ended', function () { finish(); if (repeat === 'track') { audio.currentTime = 0; audio.play(); } else if (typeof window.pwNextWord === 'function') window.pwNextWord(); }); root.querySelector('#btnPlay').onclick = function () { if (audio.paused) audio.play().catch(function () { alert('Reference audio is not available for this word yet.'); }); else audio.pause(); }; root.querySelector('#btnPrev').onclick = function () { if (window.pwPrevWord) window.pwPrevWord(); }; root.querySelector('#btnNext').onclick = function () { if (window.pwNextWord) window.pwNextWord(); }; root.querySelector('#btnShuffle').onclick = function () { shuffle = !shuffle; root.querySelector('#btnShuffle').setAttribute('aria-pressed', shuffle); }; root.querySelector('#btnRepeat').onclick = function () { repeat = repeat === 'off' ? 'queue' : repeat === 'queue' ? 'track' : 'off'; }; root.querySelector('#seek').oninput = function (e) { if (audio.duration) audio.currentTime = Number(e.target.value) / 1000 * audio.duration; }; root.querySelector('#levelPill').onclick = levels; root.querySelector('#infoBtn').onclick = function () { if (window.lgpToggleInfo) window.lgpToggleInfo(); }; }
  function clock(sec) { return isFinite(sec) ? Math.floor(sec / 60) + ':' + String(Math.floor(sec % 60)).padStart(2, '0') : '0:00'; }
  function finish() { if (!session || session.completed) return; session.completed = true; var all = sessions(); all.push(session); saveSessions(all); }
  function levels() { var choice = prompt('Choose a level from 1 to 10'); var n = Number(choice); if (!Number.isInteger(n) || n < 1 || n > 10) return; localStorage.setItem('nowssb_level_' + id(word()), String(n)); render(); }
  window.renderLiquidPlayer = render;
})();
