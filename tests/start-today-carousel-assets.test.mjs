...
assert.match(idx, /nwsb_sw_reset_v7/);
assert.match(idx, /nowssb-nm\.css\?v=804/);
assert.match(idx, /part062\.js\?v=50/);
...
assert.match(layout, /LAYOUT_V = 8/);
assert.match(nmReg, /k:'healing'[\s\S]{0,220}always:1/);
assert.doesNotMatch(nmReg, /k:'healing'[\s\S]{0,220}defOff:1/);
assert.match(nmReg, /k:'genderpath'[\s\S]{0,220}after:'healing'/);
assert.match(fashReg, /k:'healing'[\s\S]{0,220}always:1/);
assert.match(fashReg, /k:'genderpath'[\s\S]{0,220}after:'healing'/);
assert.match(layout, /if \(\(raw\.v \|\| 0\) < 8\)/);
